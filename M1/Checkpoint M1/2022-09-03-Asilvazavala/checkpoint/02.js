...
const calcularImpuestos = (factura, i = 0, acc = 0) => {
  ...
  //Si factura no es un array, retornar "Error"
  if (Array.isArray(factura) === false) return 'Error';

  //Si no tiene impuestos, retornar false
  else if(factura.length === 0) return false;

  //La función obtiene el total hallado de forma recursiva
    else if(factura.length === i) return acc;
    acc += factura[i];
    i++;
