...
// ingresa las personas validas a una nueva QUEUE, donde iremos filtrando los invitados para que solo queden 
// los invitados correctos.
//
...
//Nuevo array instanciado de la clase Queue
let admitidos = new Queue();

//Mientras haya elementos en arr continua filtrando y pushea en admitidos los que cumplen las condiciones
for (let i = 0; i < arr.length; i++) {
  if (arr[i].ticket === 'VIP' && arr[i].estado === 'Conocido') {
    admitidos.enqueue(arr[i]);
  }
}

//Si se intenta quitar un elemento de arr cuando está vacío retorna false
if (admitidos.size() === 0) return false;

//Se retornan los elementos admitidos
return admitidos;
}
