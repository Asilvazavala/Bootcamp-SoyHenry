...
module.exports = {
    ...
    cat: (args, print) => {fs.readFile(args[0], 'utf8', function(err, data){
        if(err) throw err;
        print(data)
    })},
    head: (args, print) => {fs.readFile(args[0], 'utf8', function(err, data){
        if(err) throw err;
        print(data.split('\n').splice(0, args[1]).join('\n'));
    })},
    tail: (args, print) => {fs.readFile(args[0], 'utf8', function(err, data){
        if(err) throw err;
        print(data.split('\n').splice(-args[1]).join('\n'));
    })},
    curl: (args, print) => {
