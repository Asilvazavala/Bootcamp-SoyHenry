...
/* ⚠️ No modificar nada arriba de esta línea ⚠️
  ...
  1) Los coches se van a encontrar en un array inicialmente vacío, pero a medida que se agreguen coches será un array de objetos.
...
const deleteCar = (carId) => {
  const findCar = utils.cars.find(c => c.id === carId);
  const deleteCar = utils.cars.filter(c => c.id !== carId);

  if (!findCar) { 
    throw (`No existe el coche con id: ${carId}`); 
  }
    else { 
      utils.cars = deleteCar;
    }
  return utils.cars;
};
