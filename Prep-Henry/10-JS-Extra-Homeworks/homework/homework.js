...
let arrayMayusculas = [];
  for (let i = 0; i < s.length; i++) {
    if (s[i] === s[i].toUpperCase()) arrayMayusculas.push(s[i]); 
  else {
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
let strEspejo = str.split(' ').reverse().join(' ');
let resultadoEspejo = [];

for(let i = 0; i < strEspejo.length; i++) {
  resultadoEspejo.unshift(strEspejo[i]);
}
return resultadoEspejo.join('');
}
...
function capicua(numero){
  ...
  if (numero.toString().split('').reverse().join('') === capicua) {
  return 'Es capicua';
}
