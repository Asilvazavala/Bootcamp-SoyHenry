...
function crearClaseProfesor() {
  ...
  class Profesor {
    ...
    getCertificados() {
      ...
      var arrayCertificados = Object.entries(this.certificados);
       return arrayCertificados.length; 
    }
    ...
    getUltimoCertificado() {
      ...
      for (var i = 1; i < this.certificados.length; i++) {
        var compara = this.certificados[0].anio;
        if (this.certificados[i].anio > this.certificados[0].anio) {
          compara = this.certificados[i].anio
        }
        compara = this.certificados[i];
          return compara;
      }
      }
  };
